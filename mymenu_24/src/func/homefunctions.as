	...
	try{
		if (doPreload == true){
			...
			if (merchData.length != 0){
				...
				staticmapimageurl = "http://maps.googleapis.com/maps/api/staticmap?size="+this.width.toString()+"x"+Math.round((this.height/4)).toString()+"&maptype=roadmap";
...
public function mapimageclick():void {
	...
	if ((homeData.length > 0)&&(doMove)){
		navigator.pushView(mapexample2,{homeData:homeData});
	}
}
[Bindable]
