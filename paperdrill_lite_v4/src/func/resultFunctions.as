	...
	if (ev.result.links.cited != null){
		try{
			...
			citedArray.addItem({AU:ev.result.links.cited.link.AU,
				...
				SC:ev.result.links.cited.link.SC,
				TI:ev.result.links.cited.link.TI,
				PY:ev.result.links.cited.link.PY,
	...
	if (ev.result.links.citing != null){
		try{
			...
			citingArray.addItem({AU:ev.result.links.citing.link.AU,
				...
				PY:ev.result.links.citing.link.PY,
		...
		if (ev.result.links.citing != null){
			try{
				...
				citingArray.addItem({id:ev.result.links.citing.link.id,
					level:ev.result.links.citing.link.level});
			}
		}
		else {
			citingArray = new ArrayCollection();	
...
public function afterGetSingleItem(ev:ResultEvent):void {
	try{
		...
		infoBoxDate.text = "Date: "+ev.result[0].res.re.PY;
		infoBoxHeat.text = "Heat: "+ev.result[0].res.re.heatcount.toString();
