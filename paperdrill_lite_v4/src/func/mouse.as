...
public function exportResults():void {
	...
	else if (view3.visible){
		totalText = "Author\tTitle";
		for (i = 0; i < resultArray.length; i++){
			totalText = totalText + "\r\n" + resultArray[i].AU + "\t" + resultArray[i].TI;
		}
		var fileReferenceSave:FileReference = new FileReference();
		fileReferenceSave.save(totalText,"exportedResults.txt");
	}
	else if (view4.visible){
		
	}
	else if (view5.visible){
...
public function accurateShowChange():void {
	infoBox.visible = false;
	if (view1.visible == true){
		//init();
		displayHomeSquares();
...
public function drillShowChange():void {
	infoBox.visible = false;
	df1.enabled = drillCheck.selected;
	
	arrangeType.enabled = !drillCheck.selected;
	arrangeType.selectedIndex = 0;
...
public function sortChange(ev:IndexChangeEvent):void {
	infoBox.visible = false;
	if (view1.visible == true){
		displayHomeSquares();
	}
...
public function levelToolShow(ev:MouseEvent):void {
	...
	infoBoxDate.text = "Date: "+ev.currentTarget.date;
	infoBoxHeat.text = "Heat: "+ev.currentTarget.heatcount.toString();
	infoBox.x = this.mouseX-25;
