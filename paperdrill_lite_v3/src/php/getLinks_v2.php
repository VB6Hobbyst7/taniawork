...
function workCited($s1,$n1,$n2){
   		...
   		$Return2 = "<cited>";
   		if ($au1 != ""){
   			$Query2 = "SELECT id, AU, TI from list WHERE id in (select id2 from linkage where id1 = ".$s1.")"; 					
...
function workCiting($s1,$n1,$n2){
  	$au1 = $s1;
   		...
   		$Return2 = "<citing>";
   		if ($au1 != ""){
   			$Query2 = "SELECT id, AU, TI from list WHERE id in (select id1 from linkage where id2 = ".$s1.")"; 					
  			$Result2 = mysql_query( $Query2 );
			if ($Result2){
   				while ( $User2 = mysql_fetch_object( $Result2 ))
   				{			
