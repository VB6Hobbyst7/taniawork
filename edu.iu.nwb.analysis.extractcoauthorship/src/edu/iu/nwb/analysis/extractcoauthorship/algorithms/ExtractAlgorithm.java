...
/*  82: 74 */       GraphContainer gc = GraphContainer.initializeGraph(dataTable, authorColumn, authorColumn, false, metaData, this.logger, this.progressMonitor);
					...
					String author_affiliation_string = "Authors with affiliations";
					int author_with_affiliation_index = 0;

					for (int p = 0; p < dataTable.getColumnCount(); p++){
						if (author_column_string.toLowerCase().compareTo(dataTable.getColumnName(p).toLowerCase()) == 0){
							author_index = p;
							this.logger.log(1, "Author Column found : "+dataTable.getColumnName(p));
						}
						
						if (author_affiliation_string.toLowerCase().compareTo(dataTable.getColumnName(p).toLowerCase()) == 0){
							author_with_affiliation_index = p;
							this.logger.log(1, "Authors with affiliations : "+dataTable.getColumnName(p));
						}
					}
					...
					int erorcount = 0;
					int counter = 0;
					this.logger.log(1, "Going through nodes : "+Integer.toString(dataTable.getRowCount()));
					for (int i = 0; i < outputGraph.getNodeCount(); i++){
						String authorcheck =  outputGraph.getNode(i).getString("label");
						String authorcolumn = "";
						try{
							authorcolumn = dataTable.getString(counter, author_index).toLowerCase() ;
						}
						catch(Exception e){
							counter = 0;
						}
						
						if (authorcolumn.indexOf(authorcheck.toLowerCase()) == -1){
							do{
								counter++;
								try{
									authorcolumn = dataTable.getString(counter, author_index).toLowerCase() ;
						...
						try{	
							...
							String afdata = dataTable.getString(counter, author_with_affiliation_index).toLowerCase() ;
