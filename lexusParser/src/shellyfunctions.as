...
public function loadAll():void {
	init();
	for (var i:uint = 0; i < 78; i++){
		if (i < 58){
			//normal english
			getFile(i.toString()+".txt","english");
		}
		else if (i < 65){
			//french
			getFile(i.toString()+".txt","french");
		}
		else if (i < 71){
			//german
			getFile(i.toString()+".txt","german");
		}
		/*else if (i < 76){
			//englishfactiva
			getFile2(i.toString()+".htm","english");
		}
		else if (i == 76){
			//french factiva
			getFile2(i.toString()+".htm","french");
		}
		else if (i == 77){
			//german factiva
			getFile2(i.toString()+"htm","german");
		}*/
	}
...
public function uploadSuperArray():void {
	s1 = "";
	s2 = "";
	s3 = "";
	s4 = "";
	...
	s6 = "";
	putCounter = 0;
	s1 = fixBodyText(superArray[putCounter].type);
	s2 = fixBodyText(superArray[putCounter].title);
	s3 = fixBodyText(superArray[putCounter].lang);
	s4 = superArray[putCounter].date;
	s5 = fixBodyText(superArray[putCounter].source);
	s6 = fixBodyText(superArray[putCounter].text);
	putData.send();
...
public function afterPut(ev:ResultEvent):void {
	...
	s5 = "";
	s6 = "";
	if (putCounter < superArray.length-1){
		putCounter++;
		s1 = fixBodyText(superArray[putCounter].type);
		s2 = fixBodyText(superArray[putCounter].title);
		s3 = fixBodyText(superArray[putCounter].lang);
...
public function getFile(url:String,lang:String):void {
	var file4:File = new File("app:/data2/generalother/"+url);
	...
	do {
		...
		if ((node10.indexOf("201") != -1)||(node10.indexOf("199") != -1)||(node10.indexOf("198") != -1)||(node10.indexOf("200") != -1)){
			var choice:Number = -1;
			if ((node10.indexOf("200") != -1)||(node10.indexOf("201") != -1)) {
				choice = 0;
			}
			else if ((node10.indexOf("199") != -1)||(node10.indexOf("198") != -1)) {
				choice = 1;
			}
			ts2 = ts.substring(0,
				ts.indexOf("DOCUMENTS\r",50));
			...
			if (choice == 0){
				...
				if (node5.indexOf("200") == -1){
					node5 = ts2.substring(0,ts2.indexOf("201")+4);
				}
				
			}
			else if (choice == 1){
				node5 = ts2.substring(0,ts2.indexOf("199")+4);
				if (node5.indexOf("199") == -1){
					node5 = ts2.substring(0,ts2.indexOf("198")+4);
				}
			}
			...
			if ((node5.toLowerCase().indexOf("copy") != -1)||(node10.toLowerCase().indexOf("rights") != -1)){
				...
				if ((ts2.indexOf("200") != -1)||(ts2.indexOf("201") != -1)) {
					node5 = ts2.substring(0,ts2.indexOf("200")+4);
					if (node5.indexOf("200") == -1){
						node5 = ts2.substring(0,ts2.indexOf("201")+4);
					}
		...
		try{
			...
			if (avoid == false){
				superArray.addItem({type:node1,title:node2,
					wordcount:node4,date:node5,source:node10,
					text:fixBodyText(node8),text2:"",refnum:"0",lang:lang});	
			}
		}
		catch(Exception){
...
public function getFile2(url:String,lang:String):void {
	var file4:File = new File("app:/data2/generalother/"+url);
	var fs2:FileStream = new FileStream();
	fs2.open(file4, FileMode.READ);
	var temp:String = fs2.readUTFBytes(fs2.bytesAvailable);
...
public function preBuf():void {
	var la:ArrayCollection = new ArrayCollection();
	la.addItem({name:"none"});
	la.addItem({name:"french"});
	la.addItem({name:"german"});
	landList.dataProvider = la;
	landList.selectedIndex = 0;
}
public function init():void {
	  superArray = new ArrayCollection();
	 ...
	 translateStart =0;
	  ...
	  tlist2 = "";
	  counter2 = 1;
	  lexusArray = new ArrayCollection();
	  fileloadstatus.text = "WAIT";
	pArray = new Array();
...
public function init3():void {
	//french stuff
	senderCount = Number(filestart.text);
	filesLimit = Number(fileend.text);
	//all stuff
	//senderCount = 0;
	//filesLimit = 70;
	filePath = "app:/data2/generalother/"+senderCount.toString()+".txt";
	parseFile.send();
}
...
public function gotFile(ev:ResultEvent):void {
		...
		do {
			...
			try{
				...
				if (avoid == false){
					superArray.addItem({type:node1,title:node2,
						wordcount:node4,date:node5,source:node10,
						text:fixBodyText(node8),text2:"",refnum:"0"});	
				}
			}
			catch(Exception){
	...
	if (senderCount < filesLimit){
		...
		fileloadstatus.text = "DONE";
	}
}
public function converto():void {
	if (landList.selectedIndex == 0){
		if (end1.text == "full"){
			translateStartMain = 0;
			translateStart = 0;
			translateFinish = superArray.length;
		}
		else {
			translateStartMain = Number(start1.text);
			translateStart = Number(start1.text);
			translateFinish = Number(end1.text);
		}
		createCSVFILE3();
	}
	else if (landList.selectedIndex == 1){
		if (end1.text == "full"){
			translateStartMain = 0;
			translateStart = 0;
			translateFinish = superArray.length;
	...
	else if (landList.selectedIndex == 2){
		if (end1.text == "full"){
...
public function afterTi(ev:TimerEvent):void {
	...
	if (translateFrench.url == "http://www.markbieber.ca/trans/french.php"){
		translateFrench.url = "http://www.myradar.ca/php/french.php";
	}
	else if (translateFrench.url == "http://www.myradar.ca/php/french.php"){
		translateFrench.url = "http://www.markbieber.ca/trans/french.php";
	}
...
public function createCSVFILE2():void {
	...
	file = file.resolvePath("c:/"+filenameText.text+".txt");
...
public function fixBodyText(s:String):String {
	...
	j = new RegExp("&lt;claim-text&gt;","g");
	b = b.replace(j," ");
	j = new RegExp("&lt;claim&gt;","g");
	b = b.replace(j," ");
	j = new RegExp("&lt;claims","g");
	b = b.replace(j," ");
	j = new RegExp("&lt;/","g");
	b = b.replace(j," ");
	j = new RegExp("&lt;","g");
...
public function fixDate(s:String):String {
	...
	if ((ss1.length < 3)||(ss3.length < 3)||(ss2.length < 1))
	{
		var hdhd:String = "";
		try{
			var t1:String = saveS.substring(0,saveS.indexOf(" "));
			saveS = saveS.substring(saveS.indexOf(" ")+1,saveS.length);
			var t2:String = saveS.substring(0,saveS.indexOf(" "));
			saveS = saveS.substring(saveS.indexOf(" ")+1,saveS.length);
			var t3:String = "";
			if (saveS.indexOf(" ") != -1){
				t3 = saveS.substring(0,saveS.indexOf(" "));
			}
			else {
				t3 = saveS;
			}
