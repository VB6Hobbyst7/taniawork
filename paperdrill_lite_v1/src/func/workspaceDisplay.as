...
public function displayHomeSquares():void {
	...
	if (heatCheck.selected == false){
		...
		if (v1option1.mouseEnabled == false){
				...
				for (var j:uint = 0; j < 3; j++){
					...
					if (i == 0){
						hs = new holderSquare(view1Holder.width/4-spaceVal,
							...
							homeSquareArray.date[i].cat[j],0,homeSquareArray.dateref[i],j,"a","z",imageBackVal,maxCount,minCount);
					}
					else {
						hs = new holderSquare(view1Holder.width/4-spaceVal,
							view1Holder.height/3-spaceVal,
							homeSquareArray.date[i].cat[j],homeSquareArray.dateref[i-1],homeSquareArray.dateref[i],j,"a","z",imageBackVal,maxCount,minCount);
					}
		...
		else if (v1option2.mouseEnabled == false){
...
public function displayAlphaSquares():void {
	...
	for (var i:uint = 0; i < alphaSquareArray.alpha.length; i++){
		...
		var hs:holderSquare = new holderSquare(view2Holder.width/3-spaceVal,
			...
			alphaSquareArray.alpharef[i].alphaitem[1],imageBackVal);
		hs.x = superx;
		hs.y = supery;
...
public function displayResults():void {
	...
	if (resultArray.length > 0){
		minHeat = resultArray[0].heatcount;
		maxHeat = resultArray[resultArray.length-1].heatcount;
	}
	/*for (var j:uint = 0; j < resultArray.length; j++){
		if (resultArray[j].heatcount > maxHeat){
			maxHeat = resultArray[j].heatcount;
		}
		
		if (resultArray[j].heatcount < minHeat){
			minHeat = resultArray[j].heatcount;
		}
	}*/
	if (v3option1.mouseEnabled == false){
		for (var i:uint = 0; i < resultArray.length; i++){
			...
			if (df1.selectedIndex != -1){
				if (i >= Number(df1.selectedItem.name)){
...
public function displaySearchResults():void {
	...
	if (searchresultArray.length < 1){
		...
		for (var j:uint = 0; j < searchresultArray.length; j++){
			if (searchresultArray[j].heatcount > maxHeat){
				maxHeat = searchresultArray[j].heatcount;
			}
			
			if (searchresultArray[j].heatcount < minHeat){
				minHeat = searchresultArray[j].heatcount;
			}
		}
