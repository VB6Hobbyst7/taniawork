...
public class DiagrammerModel extends VisualizerModel implements IDiagrammerModel
{
	...
	{
		_logger.info("{0}.addTable();", targetName);
		diagrammer.addTable({});
	}
	
	public function rootEdge(isOrthogonal:Boolean):void
	{
		_logger.info("{0}.rootEdge(" + isOrthogonal + ")", targetName);
		var links:Array = [];
		var link:ILink;
		for each(link in diagrammer.linksMap)
		{
			links.push(link);
		}
		diagrammer.routeEdges(links,isOrthogonal);
	}
	...
	public function removeAll():void
	{
		...
		isEmpty = true;
		_history.checkUndoRedoTasks(null);
	}
	...
	public function updateDefaultLinkLine():void
	{
		...
		if(layoutModel.layoutType == "orthogonal")
	...
	public function createGroup():void
	{
		...
		var group:IGroup = diagrammer.groupElements({label:'Title'}, diagrammer.selection.concat(),null,null,true);
		if(!group)
			return;
		group.isSelected = true;
		diagrammer.dispatchVisualizerEvent(VisualizerEvent.ELEMENTS_SELECTION_CHANGED, diagrammer.selection);
	}
