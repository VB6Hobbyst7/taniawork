...
public function view1_activateHandler(event:Event):void
{
	...
	circleToMask.graphics.beginFill(0x43c7ae,1);
...
public function topratedrecentclick(event:MouseEvent):void
{
	if (topratedrecentval == 1){
		topratedrecentval = 2;
		topratedrecentimg.source = 'assets/320/topratedrecent2.png';
		reviewList.dataProvider = recentreviews;
	}
	else {
		topratedrecentval = 1;
		topratedrecentimg.source = 'assets/320/topratedrecent1.png';
		reviewList.dataProvider = topreviews;
	}
...
public function gotobusiness():void {
	...
	try{
			...
			if (merchData.length != 0){
				for (var i:uint = 0; i < merchData.length; i++){
					if (merchData[i].id == data.locationid){
						navigator.pushView(StoresDescription, merchData[i]);	
					}
				}
			}
	}
