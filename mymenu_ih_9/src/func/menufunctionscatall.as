...
public function menuListClick():void {	
	//if (doneonece == false){
		//doneonece = true;
		//navigator.pushView(dishFullView, listData[menuList.selectedIndex]);
		//	dispatchEvent(new ActionEvent(ActionEvent.DO_ACTION, {finished:true,selectedData:listData[menuList.selectedIndex]}));
	//}
			...
			if (theatreID != -1) {
				data = e.result.Schedule.Shows.Show;
				detailNavigator.pushView(MoviesListView,data);
				detailNavigator.title = "Elokuvat tänään - "+theatreName;
			}
			else {
				data = FlexGlobals.topLevelApplication.events;
				detailNavigator.pushView(MoviesTileView,data);
				detailNavigator.title = "Kaikki elokuvat";
			}	*/
