...
public function viewadd(event:ElementExistenceEvent):void
{
	...
	else if (mainNavigator.activeView.name.toLocaleLowerCase().indexOf('settings') != -1){
		listmenu.selectedIndex = 5;
		filterfeaturebutton.visible = false;
	}
	else if (mainNavigator.activeView.name.toLocaleLowerCase().indexOf('map') != -1){
		showMap();
	}
	else {
		listmenu.selectedIndex = -1;
	...
	if (mainNavigator.activeView.name.toLocaleLowerCase().indexOf('map') == -1){
		removeallpoints();
		hideMap();
	}
}
